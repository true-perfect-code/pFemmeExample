...
namespace pFemmeExample.XUnit.UI.Components.Trends
{
    public class Trends_Button_Tests : IDisposable
    {
        ...
        public Trends_Button_Tests()
        {
            ...
            _appState.InitializeUnixTSDeviceId("test_device_ui_trends");
        ...
        private async Task CreateTestData()
        {
            ...
            var testCycle1 = new CyclesModel { RecordDate = DateTime.Now.AddDays(-60), bleeding = true, intensity = 3 };
            var testCycle2 = new CyclesModel { RecordDate = DateTime.Now.AddDays(-30), bleeding = true, intensity = 2 };
            var testCycle3 = new CyclesModel { RecordDate = DateTime.Now.AddDays(-1), bleeding = true, intensity = 4 };
        ...
        [Fact]
        public async Task Trends_RendersWithoutCrash()
        {
            ...
            ctx.Services.AddSingleton<TrendsService>(_serviceProvider.GetRequiredService<TrendsService>());
            ...
            var exception = Record.Exception(() => ctx.Render<pFemmeExample.Shared.Pages.Components.Trends>());
            Assert.Null(exception);
        }

        [Fact]
        public async Task Trends_Click_OpensModal()
        {
            await CreateTestData();
            ...
            var cut = ctx.Render<pFemmeExample.Shared.Pages.Components.Trends>();

            var component = cut.Instance;

            // Button über ID finden (statt AriaLabel)
            var button = cut.Find("#trendsButton");
            Assert.NotNull(button);
            await button.ClickAsync();

            // Modal-ID als String
            var modalId = component.ModalId.ToString();
