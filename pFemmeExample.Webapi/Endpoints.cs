...
namespace pWebApi
{
    public static class Endpoints
    {
        ...
        public static void SetEndPointsConfiguration(this WebApplication app)
        {
            ...
            try
            {
                ...
                var result = ServerConfiguration.GetSecurityConfigurationFile(pFemmeExample.Shared.Global.Configuration.ConfigGeneral);
                if (result != null)
                {
                    var pepperFilePath = !string.IsNullOrEmpty(result.out_value_str) && string.IsNullOrEmpty(result.out_err)
                        ? result.out_value_str
                        : $@"..\_Connections\{pFemmeExample.Shared.Global.Configuration.ConfigGeneral.ApplicationName}.security.config.json"; // Lokales Debug
                    try
                    {
                        // Pepper setzen
        ...
        public static async Task MapEndPoints(this WebApplication app)
        {
            var globalState = app.Services.GetRequiredService<IGlobalStateBase>();
            var email = app.Services.GetRequiredService<BlazorCore.Services.Email.IEmailBase>();
            var ai = app.Services.GetRequiredService<IAI>();

            ShowConnectionStringByError = pFemmeExample.Shared.Global.Configuration.ConfigGeneral.ShowConnectionStringByError;
            WebapiExceptionError = pFemmeExample.Shared.Global.Configuration.ConfigGeneral.WebapiExceptionError;

            // "/security/tokentpc"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_GetTokenDataTPCuser, (UserWebApi user, ISqlClientBase _sqlClient, IConfiguration config) => TokenTPCUser(user, _sqlClient, globalState, config)).AllowAnonymous();

            // "/security/changepassword"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_ChangePassword, (UserWebApi user, ISqlClientBase _sqlClient) => ChangePassword(user, _sqlClient, globalState)).RequireAuthorization();

            // "/api/spscalar"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_GetScalar, (UserWebApi user, ISqlClientBase _sqlClient) => Scalar(user, _sqlClient, globalState)).RequireAuthorization();

            // "/api/spnonquery"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_SetData, (UserWebApi user, ISqlClientBase _sqlClient) => NonQuery(user, _sqlClient, globalState)).RequireAuthorization();

            // "/api/spreader"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_GetRows, (UserWebApi user, ISqlClientBase _sqlClient) => Reader(user, _sqlClient, globalState)).RequireAuthorization();

            // "/api/anonymous"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_Anonymous, (UserWebApi user, ISqlClientBase _sqlClient) => AnonymousQuery(user, _sqlClient, globalState, email)).AllowAnonymous();

            // "/api/ai"
            app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_Ai, (UserWebApi user, IAI ai, IGlobalStateBase globalState) =>Ai(user, ai, globalState)).RequireAuthorization();


            // "/security/tokenidp"
            //app.MapPost(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_GetTokenDataIDPuser, (UserWebApi user, ISqlClientBase _sqlClient) => TokenIDPUser(user, _sqlClient, globalState)).AllowAnonymous();
    ...
    //        });
            ...
            //// "/api/mssql"
            //app.MapPost("/api/mssql", async (ISqlClientBase sqlClient) =>
            //{
            //    return await Task.FromResult($"Connected to MSSQL: {sqlClient.isConnected}");
            //}).AllowAnonymous();
            ...
            //// "/api/check"
            //app.MapGet(pFemmeExample.Shared.Global.Configuration.ConfigWebapi.endpoint_CheckToken, (ClaimsPrincipal user) =>
            //{
        ...
        public static async Task<IResult> TokenTPCUser(UserWebApi _user, ISqlClientBase _sqlClient, IGlobalStateBase globalState, IConfiguration config)
        {
            ...
            try
            {
                ...
                if (_user.EncryptDecrypt == "1")
                {
                    try
                    {
                        using (Security aes = new(pFemmeExample.Shared.Global.Configuration.ConfigGeneral.ApplicationName, pFemmeExample.Shared.Global.Configuration.ConfigGeneral.TableSchema))
                        {
                ...
                if (!String.IsNullOrEmpty(_user.JsonPara))
                {
                    ...
                    {
                        string case_ = AotConverter.ConvertTo<string>(recoveredDict["@Case_"]);
                    ...
                    {
                        string emailhash = AotConverter.ConvertTo<string>(recoveredDict["@EmailHash"]);
                        if (recoveredDict.ContainsKey("@PasswordHash"))
                        {
                            string passwordhash = AotConverter.ConvertTo<string>(recoveredDict["@PasswordHash"]);
                    ...
                    if (recoveredDict.ContainsKey("@EmailHash") && recoveredDict.ContainsKey("@PasswordHash"))
                    {
                        ...
                        if (recoveredDict.ContainsKey("@Int__Registration"))
                            registration = AotConverter.ConvertTo<string>(recoveredDict["@Int__Registration"]) == "1" ? true : false;
        ...
        public static async Task<IResult> ChangePassword(UserWebApi _user, ISqlClientBase _sqlClient, IGlobalStateBase globalState)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(_user.JsonPara))
                {
                    ...
                    if (recoveredDict.ContainsKey("@EmailHash"))
                    {
                        ChangePassword.Username = AotConverter.ConvertTo<string>(recoveredDict["@EmailHash"]);
                        if (recoveredDict.ContainsKey("@PasswordHash"))
                        {
                            ChangePassword.OldUserpassword = AotConverter.ConvertTo<string>(recoveredDict["@PasswordHash"]);
                            if (recoveredDict.ContainsKey("@PasswordHashNew"))
                            {
                                ChangePassword.NewUserpassword = AotConverter.ConvertTo<string>(recoveredDict["@PasswordHashNew"]);
                                using (pFemmeExample.Shared.Services.Security.SecurityServer aes = new())
                                {
        ...
        public static async Task<IResult> Scalar(UserWebApi _user, ISqlClientBase _sqlClient, IGlobalStateBase globalState)
        {
            ...
            {
                // Init SqlClient
                await _sqlClient.InitializeAsync(globalState);
        ...
        /// <param name="_user">Model Objekt enthält alle Informationen vom Client</param>
        /// <returns>Rückgabewert ist ScalarModel</returns>
        public static async Task<IResult> AnonymousQuery(UserWebApi _user, ISqlClientBase _sqlClient, IGlobalStateBase globalState, IEmailBase email)
        {
            ScalarModel result = new();
            bool err = false;
                        ...
                        switch (case_)
                        {
                            ...
                            case "CheckAccount>>AuthUsers":
                                try
                                {
                                    ...
                                    if (recoveredDict.ContainsKey("@EmailHash"))
                                    {
                                        string _emailHash = AotConverter.ConvertTo<string>(recoveredDict["@EmailHash"]);
                                        if (recoveredDict.ContainsKey("@PasswordHash"))
                                        {
                                            string _passwordHash = AotConverter.ConvertTo<string>(recoveredDict["@PasswordHash"]);
                                            using (pFemmeExample.Shared.Services.Security.SecurityServer aes = new())
                                            {
                                ...
                                try
                                {
                                    ...
                                    if (recoveredDict.ContainsKey("@EmailHash"))
                                    {
                                        ...
                                        if(!string.IsNullOrEmpty(emailHash))
                                        {
                                            if (recoveredDict.ContainsKey("@PasswordHash"))
                                            {
                                                string password = AotConverter.ConvertTo<string>(recoveredDict["@PasswordHash"]);
                                                using (pFemmeExample.Shared.Services.Security.SecurityServer aes = new())
                                                {
                                                    recoveredDict["@EmailHash"] = aes.HashUsername(emailHash, Pepper!); // Email hashen
                            ...
                            case "SendEmail":
                                try
                                {
                                    if (recoveredDict.ContainsKey("ToEmail")
                                        && recoveredDict.ContainsKey("Subject")
                                        && recoveredDict.ContainsKey("Body")
                                        && recoveredDict.ContainsKey("ReplyTo"))
                                    {
                                        BlazorCore.Services.Email.EmailMessageModel _data = new();

                                        _data.ToEmail = recoveredDict["ToEmail"];
                                        _data.Subject = recoveredDict["Subject"];
                                        _data.Body = recoveredDict["Body"];
                                        _data.ReplyTo = recoveredDict["ReplyTo"];

                                        await email.SendEmailAsync(_data);

                                        result.out_value_str = "1"; // Standardwert setzen
                                    }
                                }
                                catch (Exception ex)
                                {
                                    return ResponseFactory.CreateEncryptedResponse(
                                        sendResponse: err,
                                        message: $"AnonymousQuery -> switch (case_) -> try catch -> SendEmail : {ex.Message}",
                                        token: "",
                                        encryptFlag: _user.EncryptDecrypt
                                    );
                                }
        ...
        /// <summary>
        /// Führt eine AI-Operation aus (Chat, etc.) direkt über den AI Service.
        /// Authentifizierter Endpunkt (RequireAuthorization).
        /// </summary>
        /// <param name="_user">Model Objekt enthält alle Informationen vom Client (JsonPara mit AI-Parametern)</param>
        /// <param name="_ai">AI Service für direkte Ausführung</param>
        /// <param name="globalState">Global State für Deserialisierung</param>
        /// <returns>ScalarModel mit AI-Antwort in out_value_str</returns>
        public static async Task<IResult> Ai(UserWebApi _user, IAI _ai, IGlobalStateBase globalState)
        {
            ScalarModel result = new();
            bool err = false;
            ...
            try
            {
                // Entschlüsseln (nur für Windows Client)
                if (_user.EncryptDecrypt == "1")
                {
                    using (Security aes = new(
                        pFemmeExample.Shared.Global.Configuration.ConfigGeneral.ApplicationName,
                        pFemmeExample.Shared.Global.Configuration.ConfigGeneral.TableSchema))
                    {
                        _user.JsonPara = string.IsNullOrEmpty(_user.JsonPara) ? "" : aes.Decrypt(_user.JsonPara);
                    }
                }

                err = (_user.DisplayError == "1");

                if (!string.IsNullOrEmpty(_user.JsonPara))
                {
                    // Deserialisierung zu Dictionary
                    Dictionary<string, string> recoveredDict = globalState.DeserializeDictionaryTpc(_user.JsonPara);

                    // DIREKTER Aufruf des AI Service (ohne DAM Umweg)
                    result = await _ai.ExecuteAsync(recoveredDict);

                    if (!string.IsNullOrEmpty(result.out_err))
                    {
                        return ResponseFactory.CreateEncryptedResponse(
                            sendResponse: err,
                            message: $"Ai -> _ai.ExecuteAsync : {result.out_err}",
                            token: "",
                            encryptFlag: _user.EncryptDecrypt
                        );
                    }
            ...
            catch (Exception ex)
            {
                return ResponseFactory.CreateEncryptedResponse(
                    sendResponse: err,
                    message: $"Ai -> try catch : {ex.Message}",
                    token: "",
                    encryptFlag: _user.EncryptDecrypt
                );
            }

            // Verschlüsseln (nur für Windows Client)
            if (_user.EncryptDecrypt == "1")
            {
                using (Security aes = new(
                    pFemmeExample.Shared.Global.Configuration.ConfigGeneral.ApplicationName,
    ...
    public static class ResponseFactory
    {
            ...
            string encryptFlag)
        {
            ...
            if (encryptFlag == "1")
            {
                using var aes = new Security(pFemmeExample.Shared.Global.Configuration.ConfigGeneral.ApplicationName, pFemmeExample.Shared.Global.Configuration.ConfigGeneral.TableSchema);
