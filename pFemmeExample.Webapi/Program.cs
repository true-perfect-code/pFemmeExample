...
builder.Services.AddSingleton<ISqlClientBase, SqlClient>();

// Email
builder.Services.Configure<SmtpSettingsModel>(builder.Configuration.GetSection("SmtpSettings"));
builder.Services.AddSingleton<IEmailBase, pFemmeExample.Web.Services.Email>();

// Initialisierung von SQL Client => Mapping von allen Tabellen, KI
...
// SQL Client VOR Endpoint-Registrierung initialisieren
using (var scope = app.Services.CreateScope())
{
    var globalState = scope.ServiceProvider.GetRequiredService<IGlobalStateBase>();
    globalState.GlobalInit(
        pFemmeExample.Shared.Global.Configuration.ConfigGeneral,
        pFemmeExample.Shared.Global.Configuration.ConfigWebapi,
        pFemmeExample.Shared.Global.Catalog.Sections
    );

    if (pFemmeExample.Shared.Global.Configuration.ConfigGeneral.StorageLocation != STORAGE_LOCATION.LOCAL)
    {
        var sqlClient = scope.ServiceProvider.GetRequiredService<ISqlClientBase>();
...
//            {
//                string unixTS = BlazorCore.UnixTsGeneratorWebApi.Generate(pFemmeExample.Shared.Global.Configuration.ConfigGeneral);
