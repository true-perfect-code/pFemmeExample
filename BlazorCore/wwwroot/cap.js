...
window.pE_Capacitor = {
    ...
    storage: {
        ...
        /**
         * Liest eine einzelne Datei aus dem Tabellen-Ordner.
         * @param {string} dbName - Der App-Name aus C#.
         * @param {string} accountHash - Der User-Hash.
        ...
        readFile: async function (dbName, accountHash, fileName) {
            const filePath = this._getFullPath(dbName, accountHash, fileName);
            pE_Common.log("Cap:Storage", `readFile -> ${filePath}`);
            ...
            try {
                ...
                pE_Common.log("Cap:Storage", `readFile erfolgreich: ${fileName}`);
                const res = pE_Common.toScalar(fileResult.data, true, "");
                ...
                // Datei existiert nicht -> kein schwerer Fehler
                if (msg.includes("not found") || msg.includes("does not exist")) {
                    pE_Common.log("Cap:Storage", `readFile: Datei nicht gefunden: ${fileName}`);
                    const resNotFound = pE_Common.toScalar(null, true, "");
                    resNotFound.out_value_bool = true;
                    return resNotFound;
                }

                pE_Common.log("Cap:Storage", `Fehler beim Lesen von ${fileName}`, "error", e);
        ...
        /**
         * Löscht alle Dateien in einem spezifischen Tabellen-Verzeichnis für einen Benutzer.
         * @param {string} dbName - Der App-Name aus C#.
         * @param {string} accountHash - Der User-Hash.
         * @param {string} tableName - Der Name der Tabelle (z.B. "Cycles").
         */
        purgeTable: async function (dbName, accountHash, tableName) {
            // Pfad zur Tabelle: DB_Name/AccountHash/TableName
            const tablePath = this._getFullPath(dbName, accountHash, tableName);
            pE_Common.log("Cap:Storage", `purgeTable -> ${tablePath}`);

            try {
                const parent = window.pE_Capacitor;

                // Lösche das Tabellen-Verzeichnis rekursiv
                await parent.Filesystem.rmdir({
                    path: tablePath,
                    directory: 'DATA',
                    recursive: true
                ...
                // WICHTIG: Nach dem Löschen das Tabellen-Verzeichnis wieder erstellen,
                // damit die App (bzw. der File-Service) bei zukünftigen Schreibzugriffen 
                // nicht über einen fehlenden Pfad stolpert.
                await parent.Filesystem.mkdir({
                    path: tablePath,
                    directory: 'DATA',
                    recursive: true
                ...
                pE_Common.log("Cap:Storage", `Tabellen-Verzeichnis ${tableName} erfolgreich geleert.`);
                return pE_Common.toScalar(true, true, "");
            } catch (e) {
                // Falls der Ordner nicht existiert, betrachten wir das als Erfolg (Ziel erreicht)
                const msg = e.message ? e.message.toLowerCase() : "";
                if (msg.includes("not found") || msg.includes("does not exist")) {
                    pE_Common.log("Cap:Storage", "Tabellen-Verzeichnis existierte nicht - Purge als Erfolg gewertet.");

                    // Trotzdem sicherstellen, dass der Ordner für künftige Schreibvorgänge existiert
                    try {
                        await window.pE_Capacitor.Filesystem.mkdir({
                            path: tablePath,
                            directory: 'DATA',
                            recursive: true
                        });
                    } catch (mkdirError) { /* Ignorieren */ }

                    return pE_Common.toScalar(true, true, "");
                }

                pE_Common.log("Cap:Storage", `Fehler beim Purge der Tabelle ${tableName}`, "error", e);
                return pE_Common.toScalar(false, false, e.message);
            }
        }
