...
window.pE_Web = {
    ...
    notifications: {
        ...
        scheduleNotification: function (id, title, body, targetUnixSeconds) {
            ...
            try {
                ...
                //    if (Notification.permission === "granted") {
                //        // Wir definieren die Optionen mit deinem Logo-Pfad
                //        const notificationOptions = {
                //            body: body,
                //            icon: '/imgapp/logo.jpeg' // Absoluter Pfad ausgehend von wwwroot
                //        };

                //        new Notification(title, notificationOptions);

                //        pE_Common.log(`Notification mit Logo angezeigt: ${id}`);
                //    } else {
                //        pE_Common.log(`Keine Berechtigung für ID: ${id}`);
                //    }
                //    this._scheduled.delete(id);
    ...
    storage: {
        ...
        /**
         * Löscht alle Daten eines Users in dieser App.
         * @param {string} dbName - Der Name der App.
         * @param {string} accountHash - Der User-Hash.
         */
        purgeUserStorage: async function (dbName, accountHash) {
            ...
            try {
                ...
                return new Promise((resolve) => {
                    ...
                    // Bereich festlegen: Alles was mit accountHash/ beginnt
                    const range = IDBKeyRange.bound(prefix, prefix + '\uffff');
                    ...
                    request.onsuccess = (event) => {
                        ...
                        if (cursor) {
                            cursor.delete();
                            cursor.continue(); // Nächster Eintrag
                        } else {
                            // Cursor ist am Ende -> Löschen fertig
                            pE_Common.log("Web:Storage", "Purge erfolgreich abgeschlossen.");
                            resolve(pE_Common.toScalar(true, true, ""));
                        }
                    };

                    tx.oncomplete = () => {
                        // Zusätzliche Sicherheit: Transaktion komplett
                        resolve(pE_Common.toScalar(true, true, ""));
                    };

                    tx.onerror = (e) => {
                        pE_Common.log("Web:Storage", "Fehler während Purge-Transaktion", "error", e);
                        resolve(pE_Common.toScalar(false, false, e.target.error.message));
                    };
                });
            } catch (e) {
                pE_Common.log("Web:Storage", "Fataler Fehler in purgeUserStorage", "error", e);
                return pE_Common.toScalar(false, false, e.toString());
            }
        }
    },
